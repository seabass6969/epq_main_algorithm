...
def basic_noise(input, inputOutputPath, output):
    sample_rate, data = spectrogram_analysis.LoadFile(input, inputOutputPath)
    t = np.linspace(0, 20, data.shape[0])
    y = np.sin(5000 * t)

    result = data + y
    wavfile.write(os.path.join(inputOutputPath, output), sample_rate, result)

def random_noise(input, inputOutputPath, output):
    sample_rate, data = spectrogram_analysis.LoadFile(input, inputOutputPath)
